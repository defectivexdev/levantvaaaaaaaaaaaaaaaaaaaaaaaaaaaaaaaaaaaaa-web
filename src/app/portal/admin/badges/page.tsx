        ...
        <div className="max-w-7xl mx-auto p-6 space-y-6" style={{ fontFamily: "'JetBrains Mono', Consolas, monospace" }}>
            ...
            <div className="grid grid-cols-4 gap-4">
                {[
                    ...
                    <div key={s.label} className="bg-panel backdrop-blur-sm border border-white/5 rounded-xl p-4">
            ...
            <AnimatePresence>
                {showCreateForm && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="bg-panel backdrop-blur-sm border border-accent-gold/20 rounded-xl p-6 space-y-4">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder="e.g. Thailand XM-3272 Tour Finisher"
                                            className="w-full bg-[#080808] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:border-accent-gold/50 focus:outline-none" />
                                    </div>
                                    <div>
                                        ...
                                        <textarea value={formDesc} onChange={e => setFormDesc(e.target.value)} rows={3} placeholder="Award details and requirements..."
                                            className="w-full bg-[#080808] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:border-accent-gold/50 focus:outline-none resize-none" />
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <select value={formCategory} onChange={e => setFormCategory(e.target.value)}
                                            className="w-full bg-[#080808] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm appearance-none focus:border-accent-gold/50 focus:outline-none">
            ...
            <AnimatePresence>
                {showAssignForm && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="bg-panel backdrop-blur-sm border border-emerald-500/20 rounded-xl p-6 space-y-4">
                            <div className="flex items-center justify-between">
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="text" placeholder="Search pilot..." value={searchPilot} onChange={e => setSearchPilot(e.target.value)}
                                        className="w-full bg-[#080808] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm mb-2 focus:border-accent-gold/50 focus:outline-none" />
                                ...
                                <div>
                                    ...
                                    <select value={selectedAward} onChange={e => setSelectedAward(e.target.value)} className="w-full bg-[#080808] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm appearance-none focus:border-accent-gold/50 focus:outline-none mt-[42px]">
            ...
            <AnimatePresence>
                {showGrantVid && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="bg-panel backdrop-blur-sm border border-blue-500/20 rounded-xl p-6 space-y-4">
                            <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2 border-b border-white/5 pb-0">
                {(['awards', 'assignments'] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        className={`px-4 py-2.5 text-xs font-bold uppercase tracking-wider rounded-t-lg transition-colors ${tab === t ? 'bg-panel backdrop-blur-sm text-accent-gold border border-white/5 border-b-transparent -mb-px' : 'text-gray-500 hover:text-white'}`}>
                ...
                <div className="space-y-4">
                    <div className="relative max-w-md">
                        ...
                        <input type="text" placeholder="Search awards..." value={searchAward} onChange={e => setSearchAward(e.target.value)}
                            className="w-full bg-[#080808] border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:border-accent-gold/50 focus:outline-none" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredAwards.map((a: any) => (
                            <div key={a._id} className={`bg-panel backdrop-blur-sm border rounded-xl p-4 transition-colors ${a.active ? 'border-white/5 hover:border-accent-gold/20' : 'border-red-500/20 opacity-60'}`}>
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex gap-1.5 mt-2 flex-wrap">
                                            <span className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] text-gray-400 font-bold">{a.category}</span>
                                            {a.linkedTourId?.name && (
            ...
            {/* ── Assignments Table ── */}
            {tab === 'assignments' && (
                <div className="bg-panel backdrop-blur-sm border border-white/5 rounded-xl overflow-hidden">
                    <div className="p-4 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Search..." value={searchAssignment} onChange={e => setSearchAssignment(e.target.value)}
                                className="bg-[#080808] border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white w-64 focus:border-accent-gold/50 focus:outline-none" />
                        </div>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="text-left text-[10px] uppercase tracking-widest text-gray-500 bg-[#080808]/50 border-b border-white/5">
                            ...
                            <tbody>
                                {filteredAssignments.map((pa: any) => (
                                    <tr key={pa._id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="p-4">
                                            {pa.award_id?.category ? <span className="px-2 py-1 bg-white/5 border border-white/10 rounded-md text-xs text-gray-400">{pa.award_id.category}</span> : <span className="text-gray-600">—</span>}
                                        </td>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md bg-panel backdrop-blur-sm border border-red-500/20 rounded-2xl p-6 shadow-2xl text-center">
