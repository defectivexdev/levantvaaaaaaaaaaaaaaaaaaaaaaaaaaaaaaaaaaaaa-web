...
export default function OpsCenterPage() {
    ...
    return (
        <div className="space-y-2 max-w-[1600px] mx-auto h-[calc(100vh-80px)] flex flex-col overflow-hidden">
            {/* Header / Stats Bar */}
            <div className="flex flex-col lg:flex-row items-center justify-between gap-2 shrink-0">
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-accent-gold rounded-lg flex items-center justify-center animate-pulse">
                        <Activity className="text-dark-900 w-4 h-4" />
                    </div>
                    <div>
                        <h1 className="text-lg font-display font-bold text-white uppercase tracking-tight">Live Operations Center</h1>
                        <div className="flex items-center gap-2 text-[8px] font-mono text-emerald-400 uppercase tracking-widest">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5 w-full lg:w-auto">
                    <MiniStat label="Airborne" value={stats.airborne} icon={<Navigation className="text-blue-400 w-3.5 h-3.5" />} />
                    <MiniStat label="Dispatching" value={stats.preflight} icon={<Clock className="text-yellow-400 w-3.5 h-3.5" />} />
                    <MiniStat label="Total Flights" value={flights.length} icon={<Wifi className="text-emerald-400 w-3.5 h-3.5" />} />
                    <MiniStat label="Network Avg" value="99.9%" icon={<Signal className="text-purple-400 w-3.5 h-3.5" />} />
                </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-2 overflow-hidden min-h-0">
                ...
                <div className="lg:col-span-1 glass-card flex flex-col min-h-0">
                    <div className="p-2 border-b border-white/5 flex items-center justify-between">
                        <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.15em] flex items-center gap-1.5">
                            <Globe className="w-3 h-3 text-accent-gold" /> Global Flight Board
                        </h2>
                        <span className="text-[9px] font-mono text-gray-600">{flights.length} Active</span>
                    </div>
                    
                    <div className="flex-1 overflow-y-auto p-2 space-y-1.5 custom-scrollbar">
                        {flights.filter(f => f.network === 'LEVANT').map((f) => ( // Only show Levant flights in list
                            <div key={f._id} className="bg-white/[0.02] border border-white/5 rounded-lg p-2 hover:bg-white/5 transition-all group">
                                <div className="flex justify-between items-start mb-1.5">
                                    <div>
                                        <p className="text-xs font-black text-white group-hover:text-accent-gold transition-colors">{f.callsign}</p>
                                        <p className="text-[9px] text-gray-500 font-mono">{f.pilot_name}</p>
                                    </div>
                                ...
                                <div className="flex items-center justify-between gap-2 mb-1.5">
                                    <div className="text-center">
                                        <p className="text-[10px] font-bold text-white">{f.departure_icao}</p>
                                        <p className="text-[7px] text-gray-600 uppercase">DEP</p>
                                    </div>
                                    ...
                                    <div className="text-center">
                                        <p className="text-[10px] font-bold text-white">{f.arrival_icao}</p>
                                        <p className="text-[7px] text-gray-600 uppercase">ARR</p>
                                    </div>
                                </div>
                ...
                <div className="lg:col-span-3 glass-card relative overflow-hidden flex flex-col">
                    <div className="p-2 border-b border-white/5 bg-[#080808]/50 relative z-20 flex items-center justify-between">
                        <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.15em] flex items-center gap-1.5">
                    ...
                    <div className="absolute bottom-2 left-2 z-20 space-y-1.5 pointer-events-none">
                        <div className="glass-panel p-2 rounded-lg border border-white/10 max-w-[160px] backdrop-blur-2xl">
                             <p className="text-[9px] font-black text-gray-500 uppercase mb-1.5 tracking-widest">Main Hubs Status</p>
                    ...
                    <div className="absolute bottom-2 right-2 z-20 space-y-1.5 pointer-events-none max-w-[200px]">
                        {delays.length > 0 && (
                            <div className="glass-panel p-2 rounded-lg border border-orange-500/20 backdrop-blur-2xl pointer-events-auto">
                                <p className="text-[9px] font-black text-orange-400 uppercase mb-1.5 tracking-widest flex items-center gap-1">
                                    <AlertTriangle size={9} /> Current Delays
                                </p>
                                <div className="space-y-1.5">
                        ...
                        {wings.length > 0 && (
                            <div className="glass-panel p-2 rounded-lg border border-blue-500/20 backdrop-blur-2xl pointer-events-auto">
                                <p className="text-[9px] font-black text-blue-400 uppercase mb-1.5 tracking-widest flex items-center gap-1">
                                    <Users size={9} /> Flight Wings
                                </p>
                                <div className="space-y-1.5">
...
function MiniStat({ label, value, icon }: any) {
    return (
        <div className="glass-panel px-2 py-1.5 rounded-lg border border-white/5 flex items-center gap-1.5 min-w-[100px]">
            <div className="shrink-0">{icon}</div>
            <div>
                <p className="text-[7px] text-gray-500 font-bold uppercase tracking-widest leading-none mb-0.5">{label}</p>
                <p className="text-xs font-display font-black text-white leading-none">{value}</p>
            </div>
        </div>
    );
}
