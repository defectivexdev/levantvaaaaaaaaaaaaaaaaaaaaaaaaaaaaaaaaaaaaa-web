...
import { fetchAirportDetails } from '../services/airport';
...
export default function FlightPlan({ flight, telemetry, bid, pilotId, injectBid, addLogEntry, cancelFlight, submitFlight }: Props) {
  ...
  const [departureElevation, setDepartureElevation] = useState<number>(0);

  // Fetch departure airport elevation when bid changes using airportdb.io API
  useEffect(() => {
    if (!bid?.departureIcao) {
      setDepartureElevation(0);
      return;
    }
    fetchAirportDetails(bid.departureIcao).then(data => {
      if (data?.elevation_ft) {
        const elevation = typeof data.elevation_ft === 'string' ? parseFloat(data.elevation_ft) : data.elevation_ft;
        setDepartureElevation(elevation);
        console.log(`[FlightPlan] Departure elevation for ${bid.departureIcao}:`, elevation, 'ft (from airportdb.io)');
      } else {
        console.warn(`[FlightPlan] No elevation data for ${bid.departureIcao}`);
        setDepartureElevation(0);
      }
    }).catch(err => {
      console.error('[FlightPlan] Failed to fetch airport elevation:', err);
      setDepartureElevation(0);
    });
  }, [bid?.departureIcao]);
  ...
  useEffect(() => {
    ...
    // Use airport elevation + 20ft threshold for more sensitive detection
    const altitudeThreshold = departureElevation + 20;
    const hasMovement = telemetry.altitude > altitudeThreshold || telemetry.groundSpeed > 15;
    if (hasMovement) {
      console.log('[FlightPlan] Auto-start triggered — altitude:', telemetry.altitude, 'threshold:', altitudeThreshold, 'GS:', telemetry.groundSpeed);
      autoStartFiredRef.current = true;
      _autoStartFired = true;
      handleStartFlight();
      addLogEntry?.('Auto-Start: Movement detected — flight recording initiated');
    }
  }, [hasBid, bid, pilotId, expired, flight.isActive, telemetry.altitude, telemetry.groundSpeed, departureElevation, handleStartFlight, addLogEntry]);
